    ...
    asyncHandler(async (req, res) => {
        ...
        switch (true) {
            ...
            case !!categories:
                ...
                const categoryIds = categoryObjects.map(
    ...
    asyncHandler(async (req, res) => {
        ...
        switch (filterOption) {
            case "latest":
                ...
                break;
            case "high":
                const expensiveProducts = await Product.find().sort({
                    price: -1,
                });
    ...
    try {
        ...
        if (user.isBlocked) {
            ...
            if (currentTime < user.blockUntil) {
                return res
                    .status(403)
                    .send("Account blocked. Try after some time.");
            } else {
...
router.delete("/order/remove-product/:product_id", async (req, res) => {
    try {
        ...
        if (!product_id || !email) {
            return res
                .status(400)
                .json({ error: "Product ID or email not provided" });
        }
        ...
        const priceOfProductToRemove =
            ...
            (productToRemove.product.discount / 100) *
                productToRemove.product.price;
...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (order_summary.length > 0) {
            ...
            const updatedOrderSummary = await Order.findById(
                order_summary[0]._id
            )
                .populate("products.product")
                .exec();
